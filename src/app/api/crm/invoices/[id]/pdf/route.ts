...
function generateInvoiceHtml(invoice: any, paidAmount: number): string {
  ...
  const clientAddress = invoice.company?.address || invoice.company?.city || ''
  ...
  <div class="page">
    ...
    <div class="details-grid">
      ...
      <div class="detail-box">
        ...
        ${invoice.proposal ? `<p><strong>Proposal:</strong> ${invoice.proposal.title}</p>` : ''}
      </div>
    </div>
      ...
      <tbody>
        ...
        <tr>
          <td>
            ...
            ${item.description ? `<br><span style="color:#9ca3af;font-size:12px">${item.description}</span>` : ''}
          </td>
    ...
    <div class="payment-section">
      ...
      <div class="payment-item">
        <span>${formatDate(p.paidAt)} - ${p.method.replace(/_/g, ' ')}${p.reference ? ` (${p.reference})` : ''}</span>
