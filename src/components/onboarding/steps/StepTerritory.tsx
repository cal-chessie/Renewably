...
import { COUNTIES, PROVINCES } from '../onboarding-data';

export default function StepTerritory({ onNext, onBack }: { onNext: (data: Record<string, string[]>) => void; onBack: () => void }) {
  const [sel, setSel] = React.useState<string[]>(['Dublin','Kildare','Meath','Wicklow','Wexford','Louth']);
  ...
  return (
    <div className="fade-up">
      ...
      <div className="ob-territory-grid" style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 20, alignItems: 'stretch', marginBottom: 14 }}>
        <Card tone="raised" style={{ padding: 18 }}>
          <div className="mono" style={{ fontSize: 10, color: 'var(--ink-4)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>Provinces</div>
        ...
        <Card tone="solar" className="ob-territory-reach" style={{ width: 200, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div className="mono" style={{ fontSize: 9.5, color: 'var(--solar-ink)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 8 }}>Est. reach</div>
          <div className="tabular" style={{ fontSize: 38, fontWeight: 600, letterSpacing: '-0.035em', color: 'var(--solar-ink)', lineHeight: 1 }}>{sel.length}</div>
      ...
      <FooterBar>
        ...
        <Button onClick={() => onNext({ counties: sel })} disabled={sel.length === 0} icon={<IconArrow size={14} />}>
