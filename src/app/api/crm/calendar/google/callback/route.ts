  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      ...
      if (!tokenResponse.ok) {
        ...
        logger.error('Google token exchange error', { error: errData instanceof Error ? errData.message : String(errData), stack: errData instanceof Error ? errData.stack : undefined })
    ...
    if (upsertError) {
      logger.error('Failed to save Google Calendar connection', { error: upsertError.message })
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/crm/meetings?error=save_failed`)
    }

    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
    return NextResponse.redirect(`${baseUrl}/crm/meetings?connected=true`)
  } catch (error) {
    logger.error('Google callback error', { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined })
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
