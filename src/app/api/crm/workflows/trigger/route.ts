...
export async function POST(request: NextRequest) {
  try {
    ...
    if (executions.length > 0) {
      await db.workflowExecution.createMany({ data: executions })
    }
    ...
    logger.error('Trigger workflow error', { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined })
