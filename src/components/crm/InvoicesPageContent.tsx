...
function InvoiceDetail({ invoice, companyProfile, onClose, onEdit }: { invoice: Invoice; companyProfile: CompanyProfile; onClose: () => void; onEdit: (inv: Invoice) => void }) {
  ...
  const sendMutation = useMutation({ mutationFn: () => fetch(`/api/crm/invoices/${invoice.id}/send`, { method: 'POST' }).then(r => r.json()), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['invoices'] }); queryClient.invalidateQueries({ queryKey: ['invoice', invoice.id] }); toast.success('Invoice marked as sent') }, onError: () => toast.error('Failed') })
  const sendEmailMutation = useMutation({ mutationFn: async (templateId: string) => { const res = await fetch(`/api/crm/invoices/${invoice.id}/send`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ templateId }) }); if (!res.ok) throw new Error('Failed'); return res.json() }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['invoice', invoice.id] }); toast.success('Invoice marked as sent'); setEmailTemplateOpen(false) }, onError: () => toast.error('Failed') })
