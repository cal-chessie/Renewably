...
function DataErrorState({ onRetry, message }: { onRetry: () => void; message: string }) {
  return (
    <CenteredState>
      <AlertTriangle size={30} style={{ color: YELLOW }} />
      <p style={{ fontSize: 16, fontWeight: 600, color: TEXT_PRIMARY, margin: 0 }}>{message}</p>
      <p style={{ fontSize: 13, color: TEXT_SECONDARY, margin: 0 }}>Something went wrong loading this data. Try again.</p>
      <button
        onClick={onRetry}
        style={{ marginTop: 6, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 20px', borderRadius: 8, border: 'none', background: YELLOW, color: DARK, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
      >
...
function FinancialTab() {
  ...
  if (isError && !fin) return <DataErrorState onRetry={() => refetch()} message="Couldn't load financial data" />
...
// WEB PERFORMANCE TAB
// ============================================================================
function WebPerformanceEmptyState() {
  return (
    <CenteredState>
      <div style={{ width: 56, height: 56, borderRadius: 14, background: 'rgba(96,165,250,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Globe size={26} style={{ color: BLUE }} />
      </div>
      <p style={{ fontSize: 16, fontWeight: 600, color: TEXT_PRIMARY, margin: 0 }}>No website performance data</p>
      <p style={{ fontSize: 13, color: TEXT_SECONDARY, margin: 0, maxWidth: 380, lineHeight: 1.5 }}>
        Relay isn&apos;t connected to a web-analytics source yet, so there are no page-speed,
        SEO or traffic figures to show.
      </p>
    </CenteredState>
  )
}

// TODO(relay): wire this tab to a real web-analytics source (e.g. an integration
// configured under Settings). Until then it renders the honest empty state above,
// never fabricated client-website metrics.
function WebPerformanceTab() {
  return <WebPerformanceEmptyState />
}
