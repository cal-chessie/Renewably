...
import { SectionHead, Field, Input, Selector, Card, Button, FooterBar, Icon } from './ui';
...
export function StepTechnical({ onNext, onBack, initialData }: StepTechnicalProps) {
  const savedTeam = Array.isArray(initialData?.team)
    ? (initialData.team as Array<{ name: string; email: string; role: string }>).map(t => ({ n: t.name || '', e: t.email || '', r: t.role || 'Admin' }))
    : [{ n: '', e: '', r: 'Admin' as const }];

  const [team, setTeam] = useState<TeamMember[]>(savedTeam);
  const [ints, setInts] = useState<string[]>(
    Array.isArray(initialData?.tech_integrations) ? (initialData.tech_integrations as string[]) : []
  );
  const [sec, setSec] = useState<string[]>(
    Array.isArray(initialData?.security_features) ? (initialData.security_features as string[]) : []
  );
  const [retention, setRetention] = useState((initialData?.data_retention as number) || 24);
    ...
    <div className="fade-up">
      ...
      <Card style={{ padding: 16, marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div className="mono" style={{ fontSize: 10, color: 'var(--ink-4)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
            Team &middot; {team.length} seats
          </div>
          <div className="mono" style={{ fontSize: 10, color: 'var(--ink-5)' }}>
            5 included &middot; &euro;29/seat after
          </div>
        </div>
      ...
      <FooterBar>
        ...
        <Button onClick={() => onNext({
          team: team.map(t => ({ name: t.n, email: t.e, role: t.r })),
          tech_integrations: ints,
          security_features: sec,
          data_retention: retention,
        })} icon={<Icon.Arrow size={14} />}>Complete setup</Button>
      </FooterBar>
    </div>
  );
}
