...
export function StepLegal({ onNext, onBack, initialData }: StepLegalProps) {
  const [signed, setSigned] = useState<Record<string, boolean>>(
    (initialData?.signedDocs as Record<string, boolean>) || {}
  );
  const [busy, setBusy] = useState(false);
  const [viewingDoc, setViewingDoc] = useState<typeof DOCS[0] | null>(null);
  ...
  return (
    <div className="fade-up">
      <SectionHead eyebrow="05 · Legal" title="Legal agreements" desc="Review and sign to proceed. Copies will be sent to your email and stored in your portal under Settings &rarr; Documents." />
            ...
            <div key={doc.id} className="ob-doc-row" style={{
              ...
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 500, letterSpacing: '-0.01em', display: 'flex', alignItems: 'center', gap: 8 }}>
                  {doc.name}
                  {isSigned && <span className="mono" style={{ fontSize: 9, color: 'var(--green)', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '1px 5px', border: '1px solid oklch(0.76 0.17 145 / 0.3)', borderRadius: 3, background: 'var(--green-soft)' }}><Icon.Check size={9} /> Signed</span>}
                </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--line-soft)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div className="mono" style={{ fontSize: 10, color: 'var(--ink-4)' }}>{viewingDoc.pages} pages &middot; Rev. Apr 2026</div>
              </div>
              <button type="button" onClick={() => setViewingDoc(null)}
                style={{ background: 'transparent', border: 'none', color: 'var(--ink-3)', fontSize: 18, cursor: 'pointer' }}>&times;</button>
            </div>
            <div style={{ padding: 20, overflowY: 'auto', fontSize: 11.5, color: 'var(--ink-3)', lineHeight: 1.7 }}>
              ...
              <div className="img-placeholder" style={{ height: 80 }}>DOCUMENT BODY &middot; PLACEHOLDER</div>
            </div>
      ...
      <FooterBar>
        ...
        {allDone
          ? <Button onClick={() => onNext({ signedDocs: signed })} icon={<Icon.Arrow size={14} />}>Continue to finance</Button>
