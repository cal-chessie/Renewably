...
import { SectionHead, Field, Input, Selector, Card, Badge, Button, FooterBar, Icon } from './ui';
...
export function StepFinancial({ onNext, onBack, data }: StepFinancialProps) {
  const [sub, setSub] = useState(1);
  const [plan, setPlan] = useState('pro');
  const [billing, setBilling] = useState('annual');
  const setupTotal = (data && data.setupTotal) as number || 0;
  const connectedIds = (data && data.connectedIds) as string[] || [];

  // Billing detail state
  ...
  const updateBillingField = (k: string, v: string) => setBillingFields(f => ({ ...f, [k]: v }));
  ...
  if (sub === 2) return (
    <div className="fade-up">
      ...
      <div style={{ display: 'grid', gap: 14, marginBottom: 18 }}>
        <div className="ob-grid-billing" style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 10 }}>
          <Field label="VAT number"><Input value={billingFields.vat_number} onChange={v => updateBillingField('vat_number', v)} mono placeholder="IE9876543A" /></Field>
          <Field label="Invoice email" required><Input value={billingFields.invoice_email} onChange={v => updateBillingField('invoice_email', v)} placeholder="accounts@company.ie" /></Field>
        </div>
        <Field label="Billing address" required><Input value={billingFields.billing_address} onChange={v => updateBillingField('billing_address', v)} placeholder="Unit 4, Sandyford Industrial Estate" /></Field>
        <div className="ob-grid-billing" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <Field label="City"><Input value={billingFields.billing_city} onChange={v => updateBillingField('billing_city', v)} placeholder="Dublin" /></Field>
          <Field label="County"><Input value={billingFields.billing_county} onChange={v => updateBillingField('billing_county', v)} placeholder="Dublin" /></Field>
          <Field label="Eircode"><Input value={billingFields.billing_eircode} onChange={v => updateBillingField('billing_eircode', v)} mono placeholder="D18 FX78" /></Field>
        </div>
